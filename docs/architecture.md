...
| SSRF guard on `CreateShortUrl` (block private/loopback/link-local/cloud-metadata targets) | Trust the caller-supplied URL as-is | A public "create a redirect to any URL" endpoint is a classic SSRF vector (e.g. pointing it at `169.254.169.254` to reach a cloud metadata service, or at an internal admin panel). DNS resolution is abstracted behind `IDnsResolver` so this check stays testable without live network access - see `docs/testing-limitations-tradeoffs.md` for the point-in-time (TOCTOU) caveat. |
| Optional API-key gate on Create/Deactivate (`ApiKeyFilter`) | Full OAuth2/JWT per-user auth | A take-home reviewer has no auth provider to configure. The filter is opt-in via one `ApiKey` config value - unset by default so the prototype and tests need zero setup, but a real deployer turns on a real gate with one config change. A stepping stone to real per-user auth, not a replacement for it - see `docs/testing-limitations-tradeoffs.md`. |
