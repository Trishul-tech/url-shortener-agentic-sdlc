...
| `GET` | `/api/v1/orchestrator/runs/{scenario}/live/{runId}` | None | Poll a live run; returns `Running` (with the pending approval, if the pipeline is blocked on one) / `Completed` / `Failed`, and once complete, the metrics/stage-states/decision-lineage |
| `POST` | `/api/v1/orchestrator/runs/{scenario}/live/{runId}/approve` | None | Resolve the run's current pending approval as Approved |
| `POST` | `/api/v1/orchestrator/runs/{scenario}/live/{runId}/reject` | None | Resolve the run's current pending approval as Rejected (the pipeline safe-stops, matching a real rejected gate) |
| `POST` | `/api/v1/orchestrator/runs/{scenario}/live/{runId}/revise` | None | Resolve the current pending approval as Deferred with optional clarifications; the engine applies them to the run's context and genuinely re-requests approval on the same stage, bounded to 3 revisions before auto-safe-stopping |
| `GET` | `/api/v1/orchestrator/runs/{scenario}/live/{runId}/metrics` | None | Live snapshot of the run's metrics (stages attempted/succeeded, retries, rollbacks, etc.), readable while the run is still in progress, not only after completion |
| `GET` | `/api/v1/orchestrator/runs/{scenario}/live/{runId}/audit-trail` | None | Live audit event stream for the run, readable while it is still in progress |

`POST /runs/{scenario}` reuses the exact code path the console app uses - `GreenfieldScenario`/`BrownfieldScenario`/`AmbiguousScenario.Build()` plus `ScenarioRunner.RunAsync()` - executed in a background task against a real `OrchestrationEngine` and `DependencyGraph`, writing fresh artifacts to `artifacts/live-runs/{scenario}-{runId}/` (gitignored, ephemeral). Unlike the console app's scripted provider, a live run is built with `HttpApprovalProvider` (`UrlShortener.Api/Orchestration`), which genuinely blocks each approval-gated stage (Requirements, Architecture, ReleaseReadiness) on a `TaskCompletionSource` until the approve or reject endpoint resolves it - a real human-in-the-loop decision recorded in the run's decision lineage under the caller-supplied `respondedBy`, not a canned response. An unresolved approval auto-rejects after 10 minutes so an abandoned run cannot hang forever. `GET .../runs/{scenario}` (no `/live/`) still serves the checked-in `artifacts/sample-runs/` snapshot for the three required scenarios, unaffected by live runs. Live-run and pending-approval state are in-memory, scoped to the running process, and reset on restart - see `docs/testing-limitations-tradeoffs.md` for that scope. Covered by `HttpApprovalProviderTests` (resolution, rejection, no-pending-request, exposed pending state, and the timeout auto-reject).
